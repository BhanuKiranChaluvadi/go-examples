...
import (
	...
	"path"
	"path/filepath"
	"universal-robots/urcapctl/client"
...
func makeClient(cmd *cobra.Command, args []string) (*client.URCapSDKAPI, error) {
...
func makeOperationGroupUrcapCmd() (*cobra.Command, error) {
	...
	operationBuildCmd, err := makeOperationUrcapBuildCmd()
	if err != nil {
		return nil, err
	}
	operationGroupUrcapCmd.AddCommand(operationBuildCmd)

	operationDeployCmd, err := makeOperationUrcapDeployCmd()
	if err != nil {
		return nil, err
	}
	operationGroupUrcapCmd.AddCommand(operationDeployCmd)

	operationValidateCmd, err := makeOperationUrcapValidateCmd()
	if err != nil {
		return nil, err
	}
	operationGroupUrcapCmd.AddCommand(operationValidateCmd)
