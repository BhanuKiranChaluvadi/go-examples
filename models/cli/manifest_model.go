...
func registerManifestArtifacts(depth int, cmdPrefix string, cmd *cobra.Command) error {
	...
	if err := registerModelManifestArtifactsFlags(depth+1, artifactsFlagName, cmd); err != nil {
		return err
	}
...
func retrieveManifestArtifactsFlags(depth int, m *models.Manifest, cmdPrefix string, cmd *cobra.Command) (error, bool) {
	...
	if cmd.Flags().Changed(artifactsFlagName) {
		// info: complex object artifacts ManifestArtifacts is retrieved outside this Changed() block
	}
	artifactsFlagValue := m.Artifacts
	if swag.IsZero(artifactsFlagValue) {
		artifactsFlagValue = &models.ManifestArtifacts{}
	}

	err, artifactsAdded := retrieveModelManifestArtifactsFlags(depth+1, artifactsFlagValue, artifactsFlagName, cmd)
	if err != nil {
		return err, false
	}
	retAdded = retAdded || artifactsAdded
